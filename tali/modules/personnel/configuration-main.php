				...
				if ((isset($_GET['id'])) && (isset($_GET['button']))) {
					...
					switch ($_GET['button']) {
						...
						case "rank_delete":
							...
							$ftpDelete_Return = TALI_FTP_Delete($rank_image, TALI_RANKS_IMAGES_DIRECTORY);
				...
				//Add
				{
					if (isset ($_FILES['rank_file']['tmp_name'])) {
						...
						if (!$rankfile_failed) {
							...
							$ftpUpload_Return = TALI_FTP_Upload('rank_file', TALI_RANKS_IMAGES_DIRECTORY);
...
while ($db_field = mysqli_fetch_assoc($rankresult)) {
	...
	echo "
				<tr>
					...
					<td style=\"text-align:center;\"><img src=\"".TALI_DOMAIN_URL."".TALI_RANKS_IMAGES_DIRECTORY."$image\" alt=\"Rank\"></img><br/>$image</td>
